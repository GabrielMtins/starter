...
bool Texture_Load(Texture *texture, const char *filename, int cell_w, int cell_h) {
	...
	texture->num_cells_x = surface->w / cell_w;
	texture->max_id = surface->h / cell_h * texture->num_cells_x;
...
bool Texture_Render(Texture *texture, int x, int y, int id, int flip, int screen_width, int screen_height) {
	...
	Shader_SetUniform4f(
			&default_texture_shader,
			"offset_tex_coords",
			(float) ((id % texture->num_cells_x) * texture->cell_w) / texture->w,
			(float) ((id / texture->num_cells_x) * texture->cell_h) / texture->h,
			(float) texture->cell_w / texture->w,
			(float) texture->cell_h / texture->h
			);

	Texture_Use(texture, 0);
	Shader_SetUniform1i(&default_texture_shader, "frag_texture", 0);
